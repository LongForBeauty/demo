...
document.onreadystatechange = function () {
	if (document.readyState === "complete" ) {
		...
		submit_btn.className = "myBtn myBtn-dark";
		...
		$(submit_btn).click(function(){
			...
			$.post("http://127.0.0.1:8000/mindmap/SaveUserSelection/", {'text_content':user_selection,
			"user_comment":user_comment,
			"parent_node":topic,
		    "csrfmiddleware‌​token":getCookie('csrftoken')},
			function(){
				popup.style.display = "none" ;
			});
		});
...
function createBtnNodes(obj, length) {
	for (var j = 0; j < length; j++) {
		...
		newDiv.className = 'myBtn myBtn-light';
		obj.appendChild(newDiv);
	}
	//The final 'button' is left for user to input their own topic choices
	var newDiv = document.createElement('input');
	newDiv.type = 'text';
	newDiv.placeholder = 'Input your own';
	newDiv.className = 'myBtn myBtn-light';
	obj.appendChild(newDiv);
}
